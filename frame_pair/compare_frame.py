...
file_name = 'pair_image.csv'
df_right = pd.read_csv ('./frame_pair/right_df.csv').to_numpy()
df_left = (pd.read_csv ('./frame_pair/left_df.csv')).to_numpy()
...
for right in df_right:
    ...
    if (right[0] == 'f'):
        ...
        rtime = datetime.strptime(right[0], '%Y-%m-%d %H:%M:%S.%f')

        for left in df_left:
            if (left[0] == 'f'):
                print("image : " + left[1] + " failed")
            else:
                ltime = datetime.strptime(left[0], '%Y-%m-%d %H:%M:%S.%f')
                tmp_diff = 0
                if (ltime < rtime):
